...
class Cart(models.Model):
    """One cart per user (logged-in) or per guest_token (anonymous)."""
    ...
    class Meta:
        constraints = [
            # A guest_token must be unique when set
            models.UniqueConstraint(
                fields=['guest_token'],
                condition=models.Q(guest_token__isnull=False),
                name='unique_guest_token',
            )
        ]
    ...
    def __str__(self):
        ...
        return f"Cart — guest:{self.guest_token[:8]}"
    ...
    def total(self):
        ...
        return sum(item.subtotal for item in self.items.select_related('product').all()) or Decimal('0')
